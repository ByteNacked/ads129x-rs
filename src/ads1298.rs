...
pub mod conf {
    ...
    impl From<Config> for Config1Reg {
        fn from(config: Config) -> Self {
            let (high_resolution, output_date_rate) = 
            match config.mode {
        ...
        fn try_from(reg: Config1Reg) -> Result<Self, Self::Error> {
            Ok(Config{
                mode: match reg.high_resolution() {
                    true => Mode::HighResolution(SampleRateHR::try_from(reg.output_date_rate()).map_err(|_|reg.0)?),
                    false => Mode::LowPower(SampleRateLP::try_from(reg.output_date_rate()).map_err(|_|reg.0)?),
                },
